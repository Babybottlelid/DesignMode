...
class Staff{
protected:
    ControllerTower* m_controlTower;
    std::string m_name;
public:
    Staff(std::string name,ControllerTower* controlTower = nullptr):m_name(name),m_controlTower(controlTower){}
    std::string name(){return m_name;}
    void setMediator(ControllerTower* controlTower){
...
class ConcreteControTower :public ControllerTower{
private:
    Pilot* m_pilot1;
    Pilot* m_pilot2;
    Pilot* m_pilot3;
...
public:
    ConcreteControTower(Pilot* p1,Pilot* p2,Pilot* p3,GroundGrew* g1):
...
int main(){
    ...
    ConcreteControTower* controlTowe = new ConcreteControTower(p1,p2,p3,g1);
    p1->takeOff();
    g1->maintainance();
    delete p1;
    delete p2;
